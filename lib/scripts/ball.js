...
Ball.prototype = {
  createEventListeners: function () {
    this.canvas.addEventListener("ballSlimeCollision", function(e) { this.redirect(e.side); }.bind(this), false);
  ...
  redirect: function (side) {
    this.resetVelocity();
    if (side === "left") { this.redirectLeft(); }
    else if (side === "right") {this.redirectRight(); }
  },

  redirectRight: function () {
    this.updateDx(8);
    this.updateTrajectorySlope(-200);
    this.updateTrajectoryIntercept(200 + 2000);
  },

  redirectLeft: function () {
    this.updateDx(-8);
    this.updateTrajectorySlope(300);
    this.updateTrajectoryIntercept(100 - 2500);
  },
