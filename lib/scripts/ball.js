...
Ball.prototype = {
  createEventListeners: function () {
    this.canvas.addEventListener("ballSlimeCollision", function(e) { this.redirect(e.side, e.contactPoint); }.bind(this), false);
  ...
  redirect: function (side, contactPoint) {
    this.resetVelocity();
    if (side === "left") { this.redirectLeft(contactPoint); }
    else if (side === "right") {this.redirectRight(contactPoint); }
  },

  redirectRight: function (contactPoint) {
    this.updateDx(8);
    this.updateTrajectorySlope(-200);
    this.updateTrajectoryIntercept(200 + (10 * contactPoint));
  },

  redirectLeft: function (contactPoint) {
    this.updateDx(-8);
    this.updateTrajectorySlope(200);
    this.updateTrajectoryIntercept(300 + (10 * contactPoint));
  },
